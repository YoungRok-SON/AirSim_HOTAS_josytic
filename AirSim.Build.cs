...
public class AirSim : ModuleRules
{
    ...
    private void AddOSLibDependencies(ReadOnlyTargetRules Target)
    {
        ...
        {
            // 시스템 라이브러리가 위치한 Windows SDK 경로 추가
            ...
            // 시스템 라이브러리 경로를 명확히 지정
            string SystemLibPath = Path.Combine(WindowsSdkDir, "Lib", WindowsSdkVersion, "um", "x64");

            // 시스템 라이브러리 경로 추가
            PublicSystemLibraryPaths.Add(SystemLibPath);
